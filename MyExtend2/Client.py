...
class Client:
	...
	def exitClient(self):
		...
		# STATISTIC SECTION
		# self.sumOfTime += time.time() - self.beginTime
		# if self.sumOfTime > 0:
		# 	print("\n STATISTIC \n")
		# 	print('-'*40 + "\nTotal data: " + str(self.sumOfData) + " bytes \n")
		# 	print('-'*40 + "\nTotal time taken: " + str(round(self.sumOfTime,4)) +" seconds\n")
		# 	rateData = round(float(self.sumOfData/self.sumOfTime))
	...
	def listenRtp(self):		
		...
		self.beginTime = time.time() # set the time that the first RTP arrives. 
		while True: # Loop till the end of the video.
			try:
				data = self.rtpSocket.recv(20480)
				if data:
					# Get the size of the data.
					self.sumOfData += len(data)
					...
					if tempFrameNumber - 1 != self.frameNbr : # count the loss packets
						self.countPacket+=1 
										
					if tempFrameNumber > self.frameNbr: # Update the framenumber. 
